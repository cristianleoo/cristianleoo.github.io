...
const researchPapers: ResearchPaper[] = [
  {
    ...
    image: "/images/research/sirbench.png",
  ...
  return (
    <div className="group relative w-full py-10">
      ...
      <div className="overflow-x-auto px-6 md:overflow-hidden md:px-0">
        <div className="flex w-max gap-6 motion-reduce:animate-none md:gap-10 md:animate-[marquee_40s_linear_infinite] md:group-hover:[animation-play-state:paused]">
          {[...researchPapers, ...researchPapers].map((paper, i) => (
            ...
            >
              <div className="relative mb-8 aspect-[3/4] w-full overflow-hidden rounded-2xl border border-zinc-100 dark:border-zinc-800">
                {paper.image.includes('placeholder') ? (
                  <div className="flex h-full w-full flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-800/50 p-12 text-center">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 mb-4">First_Page_Preview</span>
                    <h4 className="text-sm font-bold text-zinc-600 dark:text-zinc-300 line-clamp-3">{paper.title}</h4>
                  </div>
                ) : (
                  <Image
                    src={paper.image}
